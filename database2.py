...
#also change to values
    # for key in db:
        # print(...format(db[key]["MRN"]

def main_driver():
    db = {}
    db[1] = (create_patient_entry("Ann", "Ables", 1, 34)) # new key : 1
...
def get_patient_entry(db, mrn_to_find): #search loop is slow
    # for patient in db:
        # if patient["MRN"] == mrn_to_find:
            # return patient
    # return False
    patient = db.get(mrn_to_find) #get function: if key not exist,return None
    if patient is None:
            return False
    return patient
...
def add_test_to_patient(db, mrn_to_find, test_name, test_value):
    ...
    if patient == False:
        ...
        patient["Tests"].append([test_name, test_value])#3代表第四空
    return   #表示结束 没意义
